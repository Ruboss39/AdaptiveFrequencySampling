...
error_tolerance = 1e-2;                      % Stop iteration after estimated mean relative error is below tolerance
max_iterations = 70;                         % Maximum number of iterations to perform
...
for iter = 1:max_iterations 
   ...
   if adaptive_method == "Theta1"
    ...
    % Calculate meximum estimated error
    maximum_est_error = max(mean(max_deviation,[1 2]));

    % Check if maximum estimated error is below error tolerance
    if maximum_est_error < error_tolerance   
        disp("Tolerance reached: " + string(maximum_est_error));
        break;
    end
...
% Calculate maximum of true relative error
maximum_error = max(mean(rel_error2,[1 2]));
disp("True maximum relative error: " + string(maximum_error));

% True and estimated error
rel_error_F = squeeze(pagenorm(rel_error2,"fro"));
rel_error_mean = squeeze(mean(rel_error2,[1 2]));
rel_error_F_est = squeeze(pagenorm(rel_error_est,"fro"));
rel_error_mean_est = squeeze(mean(rel_error_est,[1 2]));
...
hold on
semilogy(frequency,rel_error_F_est,'LineWidth',0.9,'Color','k','LineStyle','--');
xlim([min(frequency), max(frequency)])
xlabel('Frequency (GHz)')
ylabel('F-norm of relative error')
xlim([min(frequency), max(frequency)])
ylim([1e-6, 1e0])
grid on

figure();
semilogy(frequency,rel_error_mean,'LineWidth',0.9);
hold on
semilogy(frequency,rel_error_mean_est,'LineWidth',0.9,'Color','k','LineStyle','--');
xlim([min(frequency), max(frequency)])
xlabel('Frequency (GHz)')
